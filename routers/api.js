...
apiRouter.route("/register").post(validator.authLogin,authController.register);
...
.get(authorizeRoles(['admin','customer']), orderController.index)
.post(authorizeRoles(['admin','customer']), orderController.create);

apiRouterSecure.route("/orders/:id")
.get(authorizeRoles(['admin','customer']), orderController.read)
.put(authorizeRoles(['customer']), orderController.update)
